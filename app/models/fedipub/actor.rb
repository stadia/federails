...
module Fedipub
  ...
  class Actor < ApplicationRecord
    ...
    has_many :accepted_following_followers, -> { accepted }, class_name: 'Fedipub::Following', foreign_key: :target_actor_id, dependent: :destroy, inverse_of: :target_actor
    has_many :accepted_following_follows, -> { accepted }, class_name: 'Fedipub::Following', dependent: :destroy, inverse_of: :actor
    ...
    def shared_inbox_url
      if use_entity_attributes?
        Fedipub::Engine.routes.url_helpers.server_shared_inbox_url
      else
    ...
    #: (Fedipub::Actor) -> (Fedipub::Following | false)
    def follows?(actor)
    ...
    def sync!
      if local?
        Fedipub.logger.info 'Ignored attempt to sync a local actor'
        return false
      end
    ...
    class << self
      ...
      def find_by_account(account)
        ...
        if parts.nil? || parts[:username].blank?
          Fedipub.logger.debug { "Invalid actor account format for lookup: #{account.inspect}" }
