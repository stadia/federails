  ...
  module Server
    class ActivitiesController < Fedipub::ServerController
      ...
      def outbox
        ...
        activities = policy_scope(Fedipub::Activity, policy_scope_class: Fedipub::FedipubPolicy::Scope).where(actor: actor).order(created_at: :desc)

        render_collection(
          collection: activities,
          actor:      actor,
          url_helper: :server_actor_outbox_url
        ) { |items| Fedipub::Server::ActivityResource.new(items, params: { context: false }).serializable_hash }
      end

      # GET /federation/actors/1/activities/1.json
      def show
        render_serialized(Fedipub::Server::ActivityResource, @activity, content_type: Mime[:activitypub])
      end
      ...
      def create
        ...
        Fedipub.logger.info { "[Inbox] dispatch_request result: #{result.inspect} for activity #{payload['id']}" }
