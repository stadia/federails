  ...
  module Server
    class ActivityResource < BaseResource
      ...
      attribute :@context do |_activity|
        Fedipub::SerializerSupport.json_ld_context if include_context?
      end

      attribute :id do |activity|
        Fedipub::SerializerSupport.route_helpers.server_actor_activity_url(activity.actor, activity)
      end
      ...
      def serialize_object(entity, action: nil)
        ...
        case entity
        when Fedipub::Activity
          self.class.new(entity, params: { context: false, addressing: false }).serializable_hash
        when Fedipub::Actor
          Fedipub::Server::ActorResource.new(entity).serializable_hash.except(:@context)
        else
