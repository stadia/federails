# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `lumberjack` gem.
# Please instead update this file by running `bin/tapioca gem lumberjack`.


module ActiveSupport::TaggedLogging
  include ::Lumberjack::TaggedLogging
  extend ::Lumberjack::TaggedLogging::ClassMethods
end

# pkg:gem/lumberjack#lib/lumberjack.rb:9
module Lumberjack
  class << self
    # Contexts can be used to store tags that will be attached to all log entries in the block.
    # The context will apply to all Lumberjack loggers that are used within the block.
    #
    # If this method is called with a block, it will set a logging context for the scope of a block.
    # If there is already a context in scope, a new one will be created that inherits
    # all the tags of the parent context.
    #
    # Otherwise, it will return the current context. If one doesn't exist, it will return a new one
    # but that context will not be in any scope.
    #
    # @return [Lumberjack::Context] The current context if called without a block.
    #
    # pkg:gem/lumberjack#lib/lumberjack.rb:71
    def context(&block); end

    # Return true if inside a context block.
    #
    # @return [Boolean]
    #
    # pkg:gem/lumberjack#lib/lumberjack.rb:97
    def context?; end

    # Return the tags from the current context or nil if there are no tags.
    #
    # @return [Hash, nil]
    #
    # pkg:gem/lumberjack#lib/lumberjack.rb:104
    def context_tags; end

    # Set tags on the current context
    #
    # @param [Hash] tags The tags to set.
    # @return [void]
    #
    # pkg:gem/lumberjack#lib/lumberjack.rb:113
    def tag(tags); end

    # Define a unit of work within a block. Within the block supplied to this
    # method, calling +unit_of_work_id+ will return the same value that can
    # This can then be used for tying together log entries.
    #
    # You can specify the id for the unit of work if desired. If you don't supply
    # it, a 12 digit hexidecimal number will be automatically generated for you.
    #
    # For the common use case of treating a single web request as a unit of work, see the
    # Lumberjack::Rack::UnitOfWork class.
    #
    # @param id [String] The id for the unit of work.
    # @return [void]
    # @deprecated Use tags instead. This will be removed in version 2.0.
    #
    # pkg:gem/lumberjack#lib/lumberjack.rb:42
    def unit_of_work(id = T.unsafe(nil)); end

    # Get the UniqueIdentifier for the current unit of work.
    #
    # @return [String, nil] The id for the current unit of work.
    # @deprecated Use tags instead. This will be removed in version 2.0.
    #
    # pkg:gem/lumberjack#lib/lumberjack.rb:56
    def unit_of_work_id; end

    # Set the context to use within a block.
    #
    # @param [Lumberjack::Context] context The context to use within the block.
    # @return [Object] The result of the block.
    #
    # pkg:gem/lumberjack#lib/lumberjack.rb:84
    def use_context(context, &block); end
  end
end

# A context is used to store tags that are then added to all log entries within a block.
#
# pkg:gem/lumberjack#lib/lumberjack/context.rb:5
class Lumberjack::Context
  # @param parent_context [Context] A parent context to inherit tags from.
  #
  # pkg:gem/lumberjack#lib/lumberjack/context.rb:9
  def initialize(parent_context = T.unsafe(nil)); end

  # Get a context tag.
  #
  # @param key [String, Symbol] The tag key.
  # @return [Object] The tag value.
  #
  # pkg:gem/lumberjack#lib/lumberjack/context.rb:27
  def [](key); end

  # Set a context tag.
  #
  # @param key [String, Symbol] The tag key.
  # @param value [Object] The tag value.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/context.rb:36
  def []=(key, value); end

  # Remove tags from the context.
  #
  # @param keys [Array<String, Symbol>] The tag keys to remove.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/context.rb:44
  def delete(*keys); end

  # Clear all the context data.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/context.rb:51
  def reset; end

  # Set tags on the context.
  #
  # @param tags [Hash] The tags to set.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/context.rb:19
  def tag(tags); end

  # pkg:gem/lumberjack#lib/lumberjack/context.rb:6
  def tags; end
end

# This is an abstract class for logging devices. Subclasses must implement the +write+ method and
# may implement the +close+ and +flush+ methods if applicable.
#
# pkg:gem/lumberjack#lib/lumberjack/device.rb:6
class Lumberjack::Device
  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:110
  def cleanup_files!; end

  # Subclasses may implement this method to close the device.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device.rb:26
  def close; end

  # Subclasses may implement this method to get the format for log timestamps.
  #
  # @return [String] The format for log timestamps.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device.rb:47
  def datetime_format; end

  # Subclasses may implement this method to set a format for log timestamps.
  #
  # @param [String] format The format for log timestamps.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device.rb:54
  def datetime_format=(format); end

  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:121
  def do_once(file); end

  # Subclasses may implement this method to flush any buffers used by the device.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device.rb:41
  def flush; end

  # Subclasses may implement this method to reopen the device.
  #
  # @param [Object] logdev The log device to use.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device.rb:34
  def reopen(logdev = T.unsafe(nil)); end

  # Subclasses must implement this method to write a LogEntry.
  #
  # @param [Lumberjack::LogEntry] entry The entry to write.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device.rb:19
  def write(entry); end
end

# This log device will append entries to a file and roll the file periodically by date. Files
# are rolled at midnight and can be rolled daily, weekly, or monthly. Archive file names will
# have the date appended to them in the format ".YYYY-MM-DD" for daily, ".week-of-YYYY-MM-DD" for weekly
# and ".YYYY-MM" for monthly. It is not guaranteed that log messages will break exactly on the
# roll period as buffered entries will always be written to the same file.
#
# pkg:gem/lumberjack#lib/lumberjack/device/date_rolling_log_file.rb:12
class Lumberjack::Device::DateRollingLogFile < ::Lumberjack::Device::RollingLogFile
  # Create a new logging device to the specified file. The period to roll the file is specified
  # with the :roll option which may contain a value of :daily, :weekly,
  # or :monthly.
  #
  # @param [String, Pathname] path The path to the log file.
  # @param [Hash] options The options for the device.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/date_rolling_log_file.rb:19
  def initialize(path, options = T.unsafe(nil)); end

  # The date based suffix for file.
  #
  # @return [String]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/date_rolling_log_file.rb:34
  def archive_file_suffix; end

  # Check if the file should be rolled.
  #
  # @return [Boolean]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/date_rolling_log_file.rb:48
  def roll_file?; end

  protected

  # pkg:gem/lumberjack#lib/lumberjack/device/date_rolling_log_file.rb:69
  def after_roll; end
end

# This is a logging device that appends log entries to a file.
#
# pkg:gem/lumberjack#lib/lumberjack/device/log_file.rb:8
class Lumberjack::Device::LogFile < ::Lumberjack::Device::Writer
  # Create a logger to the file at +path+. Options are passed through to the Writer constructor.
  #
  # @param [String, Pathname] path The path to the log file.
  # @param [Hash] options The options for the device.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/log_file.rb:18
  def initialize(path, options = T.unsafe(nil)); end

  # The absolute path of the file being logged to.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/log_file.rb:12
  def path; end

  # Reopen the log file.
  #
  # @param [Object] logdev not used
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/log_file.rb:28
  def reopen(logdev = T.unsafe(nil)); end

  private

  # pkg:gem/lumberjack#lib/lumberjack/device/log_file.rb:35
  def file_stream; end
end

# pkg:gem/lumberjack#lib/lumberjack/device/log_file.rb:9
Lumberjack::Device::LogFile::EXTERNAL_ENCODING = T.let(T.unsafe(nil), String)

# This is a logging device that forward log entries to multiple other devices.
#
# pkg:gem/lumberjack#lib/lumberjack/device/multi.rb:6
class Lumberjack::Device::Multi < ::Lumberjack::Device
  # @param [Array<Lumberjack::Device>] devices The devices to write to.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/multi.rb:8
  def initialize(*devices); end

  # pkg:gem/lumberjack#lib/lumberjack/device/multi.rb:24
  def close; end

  # pkg:gem/lumberjack#lib/lumberjack/device/multi.rb:36
  def datetime_format; end

  # pkg:gem/lumberjack#lib/lumberjack/device/multi.rb:40
  def datetime_format=(format); end

  # pkg:gem/lumberjack#lib/lumberjack/device/multi.rb:18
  def flush; end

  # pkg:gem/lumberjack#lib/lumberjack/device/multi.rb:30
  def reopen(logdev = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/device/multi.rb:12
  def write(entry); end
end

# This is a logging device that produces no output. It can be useful in
# testing environments when log file output is not useful.
#
# pkg:gem/lumberjack#lib/lumberjack/device/null.rb:7
class Lumberjack::Device::Null < ::Lumberjack::Device
  # pkg:gem/lumberjack#lib/lumberjack/device/null.rb:8
  def initialize(*args); end

  # pkg:gem/lumberjack#lib/lumberjack/device/null.rb:11
  def write(entry); end
end

# This is an abstract class for a device that appends entries to a file and periodically archives
# the existing file and starts a one. Subclasses must implement the roll_file? and archive_file_suffix
# methods.
#
# The :keep option can be used to specify a maximum number of rolled log files to keep.
# Older files will be deleted based on the time they were created. The default is to keep all files.
#
# The :min_roll_check option can be used to specify the number of seconds between checking
# the file to determine if it needs to be rolled. The default is to check at most once per second.
#
# pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:14
class Lumberjack::Device::RollingLogFile < ::Lumberjack::Device::LogFile
  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:18
  def initialize(path, options = T.unsafe(nil)); end

  # Returns a suffix that will be appended to the file name when it is archived.. The suffix should
  # change after it is time to roll the file. The log file will be renamed when it is rolled.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:34
  def archive_file_suffix; end

  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:16
  def keep; end

  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:16
  def keep=(_arg0); end

  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:15
  def path; end

  # Roll the log file by renaming it to the archive file name and then re-opening a stream to the log
  # file path. Rolling a file is safe in multi-threaded or multi-process environments.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:45
  def roll_file!; end

  # Return +true+ if the file should be rolled.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:39
  def roll_file?; end

  protected

  # This method will be called after a file has been rolled. Subclasses can
  # implement code to reset the state of the device. This method is thread safe.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:73
  def after_roll; end

  # Handle rolling the file before flushing.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:77
  def before_flush; end

  private

  # pkg:gem/lumberjack#lib/lumberjack/device/rolling_log_file.rb:96
  def reopen_file; end
end

# This is a log device that appends entries to a file and rolls the file when it reaches a specified
# size threshold. When a file is rolled, it will have an number extension appended to the file name.
# For example, if the log file is named production.log, the first time it is rolled it will be renamed
# production.log.1, then production.log.2, etc.
#
# pkg:gem/lumberjack#lib/lumberjack/device/size_rolling_log_file.rb:9
class Lumberjack::Device::SizeRollingLogFile < ::Lumberjack::Device::RollingLogFile
  # Create an new log device to the specified file. The maximum size of the log file is specified with
  # the :max_size option. The unit can also be specified: "32K", "100M", "2G" are all valid.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/size_rolling_log_file.rb:14
  def initialize(path, options = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/device/size_rolling_log_file.rb:38
  def archive_file_suffix; end

  # pkg:gem/lumberjack#lib/lumberjack/device/size_rolling_log_file.rb:10
  def max_size; end

  # pkg:gem/lumberjack#lib/lumberjack/device/size_rolling_log_file.rb:42
  def roll_file?; end

  protected

  # Calculate the next archive file name extension.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/size_rolling_log_file.rb:51
  def next_archive_number; end
end

# This logging device writes log entries as strings to an IO stream. By default, messages will be buffered
# and written to the stream in a batch when the buffer is full or when +flush+ is called.
#
# Subclasses can implement a +before_flush+ method if they have logic to execute before flushing the log.
# If it is implemented, it will be called before every flush inside a mutex lock.
#
# pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:10
class Lumberjack::Device::Writer < ::Lumberjack::Device
  # Create a new device to write log entries to a stream. Entries are converted to strings
  # using a Template. The template can be specified using the :template option. This can
  # either be a Proc or a string that will compile into a Template object.
  #
  # If the template is a Proc, it should accept an LogEntry as its only argument and output a string.
  #
  # If the template is a template string, it will be used to create a Template. The
  # :additional_lines and :time_format options will be passed through to the
  # Template constuctor.
  #
  # The default template is "[:time :severity :progname(:pid)] :message"
  # with additional lines formatted as "\n  :message".
  #
  # The size of the internal buffer in bytes can be set by providing :buffer_size (defaults to 32K).
  #
  # @param [IO] stream The stream to write log entries to.
  # @param [Hash] options The options for the device.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:65
  def initialize(stream, options = T.unsafe(nil)); end

  # The size of the internal buffer. Defaults to 32K.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:15
  def buffer_size; end

  # Set the buffer size in bytes. The device will only be physically written to when the buffer size
  # is exceeded.
  #
  # @param [Integer] value The size of the buffer in bytes.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:85
  def buffer_size=(value); end

  # Close the underlying stream.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:118
  def close; end

  # Get the datetime format.
  #
  # @return [String] The datetime format.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:138
  def datetime_format; end

  # Set the datetime format.
  #
  # @param [String] format The datetime format.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:146
  def datetime_format=(format); end

  # Return the underlying stream. Provided for API compatibility with Logger devices.
  #
  # @return [IO] The underlying stream.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:155
  def dev; end

  # Flush the underlying stream.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:126
  def flush; end

  # Write an entry to the stream. The entry will be converted into a string using the defined template.
  #
  # @param [LogEntry, String] entry The entry to write to the stream.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:94
  def write(entry); end

  protected

  # Set the underlying stream.
  # Get the underlying stream.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:165
  def stream; end

  # Set the underlying stream.
  # Get the underlying stream.
  #
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:162
  def stream=(_arg0); end

  private

  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:169
  def write_to_stream(lines); end
end

# Internal buffer to batch writes to the stream.
#
# pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:18
class Lumberjack::Device::Writer::Buffer
  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:21
  def initialize; end

  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:26
  def <<(string); end

  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:42
  def clear; end

  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:31
  def empty?; end

  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:35
  def pop!; end

  # pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:19
  def size; end
end

# pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:12
Lumberjack::Device::Writer::DEFAULT_ADDITIONAL_LINES_TEMPLATE = T.let(T.unsafe(nil), String)

# pkg:gem/lumberjack#lib/lumberjack/device/writer.rb:11
Lumberjack::Device::Writer::DEFAULT_FIRST_LINE_TEMPLATE = T.let(T.unsafe(nil), String)

# This class controls the conversion of log entry messages into a loggable format. This allows you
# to log any object you want and have the logging system deal with converting it into a string.
#
# Formats are added to a Formatter by associating them with a class using the +add+ method. Formats
# are any object that responds to the +call+ method.
#
# By default, all object will be converted to strings using their inspect method except for Strings
# and Exceptions. Strings are not converted and Exceptions are converted using the ExceptionFormatter.
#
# Enumerable objects (including Hash and Array) will call the formatter recursively for each element.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter.rb:14
class Lumberjack::Formatter
  # pkg:gem/lumberjack#lib/lumberjack/formatter.rb:41
  def initialize; end

  # Add a formatter for a class. The formatter can be specified as either an object
  # that responds to the +call+ method or as a symbol representing one of the predefined
  # formatters, or as a block to the method call.
  #
  # The predefined formatters are:
  #   - :date_time
  #   - :exception
  #   - :id
  #   - :inspect
  #   - :object
  #   - :pretty_print
  #   - :string
  #   - :strip
  #   - :structured
  #   - :truncate
  #
  # You can add multiple classes at once by passing an array of classes.
  #
  # You can also pass class names as strings instead of the classes themselves. This can
  # help avoid loading dependency issues. This applies only to classes; modules cannot be
  # passed in as strings.
  #
  # @param klass [Class, Module, String, Array<Class, Module, String>] The class or module to add a formatter for.
  # @param formatter [Symbol, Class, String, #call] The formatter to use for the class.
  #   If a symbol is passed in, it will be used to load one of the predefined formatters.
  #   If a class is passed in, it will be initialized with the args passed in.
  #   Otherwise, the object will be used as the formatter and must respond to call method.
  # @param args [Array] Arguments to pass to the formatter when it is initialized.
  # @yield [obj] A block that will be used as the formatter for the class.
  # @yieldparam [Object] obj The object to format.
  # @yieldreturn [String] The formatted string.
  # @return [self] Returns itself so that add statements can be chained together.
  #
  # @example
  #
  #   # Use a predefined formatter
  #   formatter.add(MyClass, :pretty_print)
  #
  #   # Pass in a formatter object
  #   formatter.add(MyClass, Lumberjack::Formatter::PrettyPrintFormatter.new)
  #
  #   # Use a block
  #   formatter.add(MyClass){|obj| obj.humanize}
  #
  #   # Add statements can be chained together
  #   formatter.add(MyClass, :pretty_print).add(YourClass){|obj| obj.humanize}
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter.rb:97
  def add(klass, formatter = T.unsafe(nil), *args, &block); end

  # Compatibility with the Logger::Formatter signature. This method will just convert the message
  # object to a string and ignores the other parameters.
  #
  # @param severity [Integer, String, Symbol] The severity of the message.
  # @param timestamp [Time] The time the message was logged.
  # @param progname [String] The name of the program logging the message.
  # @param msg [Object] The message object to format.
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter.rb:193
  def call(severity, timestamp, progname, msg); end

  # Remove all formatters including the default formatter. Can be chained to add method calls.
  #
  # @return [self] Returns itself so that clear statements can be chained together.
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter.rb:153
  def clear; end

  # Return true if their are no registered formatters.
  #
  # @return [Boolean] true if there are no registered formatters, false otherwise.
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter.rb:162
  def empty?; end

  # Format a message object by applying all formatters attached to it.
  #
  # @param message [Object] The message object to format.
  # @return [Object] The formatted object.
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter.rb:170
  def format(message); end

  # Find the formatter for a class by looking it up using the class hierarchy.
  #
  # @api private
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter.rb:202
  def formatter_for(klass); end

  # Remove the formatter associated with a class. Remove statements can be chained together.
  #
  # You can remove multiple classes at once by passing an array of classes.
  #
  # You can also pass class names as strings instead of the classes themselves. This can
  # help avoid loading dependency issues. This applies only to classes; modules cannot be
  # passed in as strings.
  #
  # @param klass [Class, Module, String, Array<Class, Module, String>] The class or module to remove the formatters for.
  # @return [self] Returns itself so that remove statements can be chained together.
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter.rb:138
  def remove(klass); end

  class << self
    # Returns a new empty formatter with no mapping. For historical reasons, a formatter
    # is initialized with mappings to help output objects as strings. This will return one
    # without the default mappings.
    #
    # @return [Lumberjack::Formatter] a new empty formatter
    #
    # pkg:gem/lumberjack#lib/lumberjack/formatter.rb:36
    def empty; end
  end
end

# Format a Date, Time, or DateTime object. If you don't specify a format in the constructor,
# it will use the ISO-8601 format.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/date_time_formatter.rb:7
class Lumberjack::Formatter::DateTimeFormatter
  # @param [String] format The format to use when formatting the date/time object.
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter/date_time_formatter.rb:11
  def initialize(format = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/date_time_formatter.rb:15
  def call(obj); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/date_time_formatter.rb:8
  def format; end
end

# Format an exception including the backtrace. You can specify an object that
# responds to `call` as a backtrace cleaner. The exception backtrace will be
# passed to this object and the returned array is what will be logged. You can
# use this to clean out superfluous lines.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/exception_formatter.rb:9
class Lumberjack::Formatter::ExceptionFormatter
  # @param [#call] backtrace_cleaner An object that responds to `call` and takes
  #   an array of strings (the backtrace) and returns an array of strings (the
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter/exception_formatter.rb:14
  def initialize(backtrace_cleaner = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/exception_formatter.rb:10
  def backtrace_cleaner; end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/exception_formatter.rb:10
  def backtrace_cleaner=(_arg0); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/exception_formatter.rb:18
  def call(exception); end

  private

  # pkg:gem/lumberjack#lib/lumberjack/formatter/exception_formatter.rb:30
  def clean_backtrace(trace); end
end

# Format an object that has an id as a hash with keys for class and id. This formatter is useful
# as a default formatter for objects pulled from a data store. By default it will use :id as the
# id attribute.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/id_formatter.rb:8
class Lumberjack::Formatter::IdFormatter
  # @param [Symbol, String] id_attribute The attribute to use as the id.
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter/id_formatter.rb:10
  def initialize(id_attribute = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/id_formatter.rb:14
  def call(obj); end
end

# Format an object by calling +inspect+ on it.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/inspect_formatter.rb:6
class Lumberjack::Formatter::InspectFormatter
  # pkg:gem/lumberjack#lib/lumberjack/formatter/inspect_formatter.rb:7
  def call(obj); end
end

# This formatter can be used to multiply a numeric value by a specified multiplier and
# optionally round to a specified number of decimal places.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/multiply_formatter.rb:7
class Lumberjack::Formatter::MultiplyFormatter
  # @param multiplier [Numeric] The multiplier to apply to the value.
  # @param decimals [Integer, nil] The number of decimal places to round the result to.
  #   If nil, no rounding is applied.
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter/multiply_formatter.rb:11
  def initialize(multiplier, decimals = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/multiply_formatter.rb:16
  def call(value); end
end

# No-op formatter that just returns the object itself.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/object_formatter.rb:6
class Lumberjack::Formatter::ObjectFormatter
  # pkg:gem/lumberjack#lib/lumberjack/formatter/object_formatter.rb:7
  def call(obj); end
end

# Format an object with it's pretty print method.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/pretty_print_formatter.rb:9
class Lumberjack::Formatter::PrettyPrintFormatter
  # Create a new formatter. The maximum width of the message can be specified with the width
  # parameter (defaults to 79 characters).
  #
  # @param [Integer] width The maximum width of the message.
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter/pretty_print_formatter.rb:16
  def initialize(width = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/pretty_print_formatter.rb:20
  def call(obj); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/pretty_print_formatter.rb:10
  def width; end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/pretty_print_formatter.rb:10
  def width=(_arg0); end
end

# Log sensitive information in a redacted format showing the firat and last
# characters of the value, with the rest replaced by asterisks. The number of
# characters shown is dependent onthe length of the value; short values will
# not show any characters in order to avoid revealing too much information.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/redact_formatter.rb:9
class Lumberjack::Formatter::RedactFormatter
  # pkg:gem/lumberjack#lib/lumberjack/formatter/redact_formatter.rb:10
  def call(obj); end
end

# Round numeric values to a set number of decimal places. This is useful when logging
# floating point numbers to reduce noise and rounding errors in the logs.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/round_formatter.rb:7
class Lumberjack::Formatter::RoundFormatter
  # pkg:gem/lumberjack#lib/lumberjack/formatter/round_formatter.rb:8
  def initialize(precision = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/round_formatter.rb:12
  def call(obj); end
end

# Format an object by calling `to_s` on it.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/string_formatter.rb:6
class Lumberjack::Formatter::StringFormatter
  # pkg:gem/lumberjack#lib/lumberjack/formatter/string_formatter.rb:7
  def call(obj); end
end

# Format an object by calling `to_s` on it and stripping leading and trailing whitespace.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/strip_formatter.rb:6
class Lumberjack::Formatter::StripFormatter
  # pkg:gem/lumberjack#lib/lumberjack/formatter/strip_formatter.rb:7
  def call(obj); end
end

# Dereference arrays and hashes and recursively call formatters on each element.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/structured_formatter.rb:8
class Lumberjack::Formatter::StructuredFormatter
  # @param [Formatter] formatter The formatter to call on each element
  #   in the structure.
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter/structured_formatter.rb:14
  def initialize(formatter = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/structured_formatter.rb:18
  def call(obj); end

  private

  # pkg:gem/lumberjack#lib/lumberjack/formatter/structured_formatter.rb:24
  def call_with_references(obj, references); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/structured_formatter.rb:50
  def with_object_reference(obj, references); end
end

# pkg:gem/lumberjack#lib/lumberjack/formatter/structured_formatter.rb:9
class Lumberjack::Formatter::StructuredFormatter::RecusiveReferenceError < ::StandardError; end

# This class can be used as the return value from a formatter `call` method to
# extract additional tags from an object being logged. This can be useful when there
# using structured logging to include important metadata in the log message.
#
# @example
#  # Automatically add tags with error details when logging an exception.
#  logger.add_formatter(Exception, ->(e) {
#    Lumberjack::Formatter::TaggedMessage.new(e.message, {
#      error: {
#        message: e.message,
#        class: e.class.name,
#        trace: e.backtrace
#      }
#    })
#  })
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/tagged_message.rb:20
class Lumberjack::Formatter::TaggedMessage
  # @param [Formatter] formatter The formatter to apply the transformation to.
  # @param [Proc] transform The transformation function to apply to the formatted string.
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter/tagged_message.rb:25
  def initialize(message, tags); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/tagged_message.rb:34
  def inspect; end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/tagged_message.rb:21
  def message; end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/tagged_message.rb:21
  def tags; end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/tagged_message.rb:30
  def to_s; end
end

# Truncate a string object to a specific length. This is useful
# for formatting messages when there is a limit on the number of
# characters that can be logged per message. This formatter should
# only be used when necessary since it is a lossy formatter.
#
# When a string is truncated, it will have a unicode ellipsis
# character (U+2026) appended to the end of the string.
#
# pkg:gem/lumberjack#lib/lumberjack/formatter/truncate_formatter.rb:12
class Lumberjack::Formatter::TruncateFormatter
  # @param [Integer] length The maximum length of the string (defaults to 32K)
  #
  # pkg:gem/lumberjack#lib/lumberjack/formatter/truncate_formatter.rb:14
  def initialize(length = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/formatter/truncate_formatter.rb:18
  def call(obj); end
end

# pkg:gem/lumberjack#lib/lumberjack.rb:10
Lumberjack::LINE_SEPARATOR = T.let(T.unsafe(nil), String)

# An entry in a log is a data structure that captures the log message as well as
# information about the system that logged the message.
#
# pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:6
class Lumberjack::LogEntry
  # Create a new log entry.
  #
  # @param time [Time] The time the log entry was created.
  # @param severity [Integer, String] The severity of the log entry.
  # @param message [String] The message to log.
  # @param progname [String] The name of the program that created the log entry.
  # @param pid [Integer] The process id of the program that created the log entry.
  # @param tags [Hash<String, Object>] A hash of tags to associate with the log entry.
  #
  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:22
  def initialize(time, severity, message, progname, pid, tags); end

  # Return true if the log entry has no message and no tags.
  #
  # @return [Boolean] True if the log entry is empty, false otherwise.
  #
  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:91
  def empty?; end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:44
  def inspect; end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def message; end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def message=(_arg0); end

  # Helper method to expand the tags into a nested structure. Tags with dots in the name
  # will be expanded into nested hashes.
  #
  # @return [Hash] The tags expanded into a nested structure.
  #
  # @example
  #   entry = Lumberjack::LogEntry.new(Time.now, Logger::INFO, "test", "app", 1500, "a.b.c" => 1, "a.b.d" => 2)
  #   entry.nested_tags # => {"a" => {"b" => {"c" => 1, "d" => 2}}}
  #
  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:84
  def nested_tags; end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def pid; end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def pid=(_arg0); end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def progname; end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def progname=(_arg0); end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def severity; end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def severity=(_arg0); end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:36
  def severity_label; end

  # Return the tag with the specified name.
  #
  # @param name [String, Symbol] The tag name.
  # @return [Object, nil] The tag value or nil if the tag does not exist.
  #
  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:70
  def tag(name); end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def tags; end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def tags=(_arg0); end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def time; end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:7
  def time=(_arg0); end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:40
  def to_s; end

  # @deprecated - backward compatibility with 1.0 API. Will be removed in version 2.0.
  #
  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:49
  def unit_of_work_id; end

  # @deprecated - backward compatibility with 1.0 API. Will be removed in version 2.0.
  #
  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:56
  def unit_of_work_id=(value); end

  private

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:103
  def compact_tags(tags, seen = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:97
  def tags_to_s; end
end

# pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:9
Lumberjack::LogEntry::TIME_FORMAT = T.let(T.unsafe(nil), String)

# @deprecated Will be removed in version 2.0.
#
# pkg:gem/lumberjack#lib/lumberjack/log_entry.rb:12
Lumberjack::LogEntry::UNIT_OF_WORK_ID = T.let(T.unsafe(nil), String)

# Logger is a thread safe logging object. It has a compatible API with the Ruby
# standard library Logger class, the Log4r gem, and ActiveSupport::BufferedLogger.
#
# === Example
#
#   logger = Lumberjack::Logger.new
#   logger.info("Starting processing")
#   logger.debug("Processing options #{options.inspect}")
#   logger.fatal("OMG the application is on fire!")
#
# Log entries are written to a logging Device if their severity meets or exceeds the log level.
#
# Devices may use buffers internally and the log entries are not guaranteed to be written until you call
# the +flush+ method. Sometimes this can result in problems when trying to track down extraordinarily
# long running sections of code since it is likely that none of the messages logged before the long
# running code will appear in the log until the entire process finishes. You can set the +:flush_seconds+
# option on the constructor to force the device to be flushed periodically. This will create a new
# monitoring thread, but its use is highly recommended.
#
# Each log entry records the log message and severity along with the time it was logged, the
# program name, process id, and an optional hash of tags. The message will be converted to a string, but
# otherwise, it is up to the device how these values are recorded. Messages are converted to strings
# using a Formatter associated with the logger.
#
# pkg:gem/lumberjack#lib/lumberjack/logger.rb:27
class Lumberjack::Logger
  include ::Lumberjack::Severity

  # Create a new logger to log to a Device.
  #
  # The +device+ argument can be in any one of several formats.
  #
  # If it is a Device object, that object will be used.
  # If it has a +write+ method, it will be wrapped in a Device::Writer class.
  # If it is :null, it will be a Null device that won't record any output.
  # Otherwise, it will be assumed to be file path and wrapped in a Device::LogFile class.
  #
  # All other options are passed to the device constuctor.
  #
  # @param [Lumberjack::Device, Object, Symbol, String] device The device to log to.
  # @param [Hash] options The options for the logger.
  # @option options [Integer, Symbol, String] :level The logging level below which messages will be ignored.
  # @option options [Lumberjack::Formatter] :formatter The formatter to use for outputting messages to the log.
  # @option options [String] :datetime_format The format to use for log timestamps.
  # @option options [Lumberjack::Formatter] :message_formatter The MessageFormatter to use for formatting log messages.
  # @option options [Lumberjack::TagFormatter] :tag_formatter The TagFormatter to use for formatting tags.
  # @option options [String] :progname The name of the program that will be recorded with each log entry.
  # @option options [Numeric] :flush_seconds The maximum number of seconds between flush calls.
  # @option options [Boolean] :roll If the log device is a file path, it will be a Device::DateRollingLogFile if this is set.
  # @option options [Integer] :max_size If the log device is a file path, it will be a Device::SizeRollingLogFile if this is set.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:67
  def initialize(device = T.unsafe(nil), options = T.unsafe(nil)); end

  # Add a message when the severity is not known.
  #
  # @param [Object] msg The message to log.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:437
  def <<(msg); end

  # ::Logger compatible method to add a log entry.
  #
  # @param [Integer, Symbol, String] severity The severity of the message.
  # @param [Object] message The message to log.
  # @param [String] progname The name of the program that is logging the message.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:249
  def add(severity, message = T.unsafe(nil), progname = T.unsafe(nil), &block); end

  # Add a message to the log with a given severity. The message can be either
  # passed in the +message+ argument or supplied with a block. This method
  # is not normally called. Instead call one of the helper functions
  # +fatal+, +error+, +warn+, +info+, or +debug+.
  #
  # The severity can be passed in either as one of the Severity constants,
  # or as a Severity label.
  #
  # @param [Integer, Symbol, String] severity The severity of the message.
  # @param [Object] message The message to log.
  # @param [String] progname The name of the program that is logging the message.
  # @param [Hash] tags The tags to add to the log entry.
  # @return [void]
  #
  # @example
  #
  #   logger.add_entry(Logger::ERROR, exception)
  #   logger.add_entry(Logger::INFO, "Request completed")
  #   logger.add_entry(:warn, "Request took a long time")
  #   logger.add_entry(Logger::DEBUG){"Start processing with options #{options.inspect}"}
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:202
  def add_entry(severity, message, progname = T.unsafe(nil), tags = T.unsafe(nil)); end

  # Close the logging device.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:275
  def close; end

  # Returns +true+ if the logging device is closed.
  #
  # @return [Boolean] +true+ if the logging device is closed.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:284
  def closed?; end

  # Set up a context block for the logger. All tags added within the block will be cleared when
  # the block exits.
  #
  # @param [Proc] block The block to execute with the tag context.
  # @return [TagContext] If no block is passed, then a Lumberjack::TagContext is returned that can be used
  #   to interact with the tags (add, remove, etc.).
  # @yield [TagContext] If a block is passed, it will be yielded a TagContext object that can be used to
  #   add or remove tags within the context.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:534
  def context(&block); end

  # Get the timestamp format on the device if it has one.
  #
  # @return [String, nil] The timestamp format or nil if the device doesn't support it.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:105
  def datetime_format; end

  # Set the timestamp format on the device if it is supported.
  #
  # @param [String] format The timestamp format.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:113
  def datetime_format=(format); end

  # Log a +DEBUG+ message. The message can be passed in either the +message+ argument or in a block.
  #
  # @param [Object] message_or_progname_or_tags The message to log or progname
  #   if the message is passed in a block.
  # @param [String, Hash] progname_or_tags The name of the program that is logging the message or tags
  #   if the message is passed in a block.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:403
  def debug(message_or_progname_or_tags = T.unsafe(nil), progname_or_tags = T.unsafe(nil), &block); end

  # Set the log level to debug.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:417
  def debug!; end

  # Return +true+ if +DEBUG+ messages are being logged.
  #
  # @return [Boolean]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:410
  def debug?; end

  # Get the logging device that is used to write log entries.
  #
  # @return [Lumberjack::Device] The logging device.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:90
  def device; end

  # Set the logging device to a new device.
  #
  # @param [Lumberjack::Device] device The new logging device.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:98
  def device=(device); end

  # Log an +ERROR+ message. The message can be passed in either the +message+ argument or in a block.
  #
  # @param [Object] message_or_progname_or_tags The message to log or progname
  #   if the message is passed in a block.
  # @param [String, Hash] progname_or_tags The name of the program that is logging the message or tags
  #   if the message is passed in a block.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:328
  def error(message_or_progname_or_tags = T.unsafe(nil), progname_or_tags = T.unsafe(nil), &block); end

  # Set the log level to error.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:342
  def error!; end

  # Return +true+ if +ERROR+ messages are being logged.
  #
  # @return [Boolean]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:335
  def error?; end

  # Log a +FATAL+ message. The message can be passed in either the +message+ argument or in a block.
  #
  # @param [Object] message_or_progname_or_tags The message to log or progname
  #   if the message is passed in a block.
  # @param [String, Hash] progname_or_tags The name of the program that is logging the message or tags
  #   if the message is passed in a block.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:303
  def fatal(message_or_progname_or_tags = T.unsafe(nil), progname_or_tags = T.unsafe(nil), &block); end

  # Set the log level to fatal.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:317
  def fatal!; end

  # Return +true+ if +FATAL+ messages are being logged.
  #
  # @return [Boolean]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:310
  def fatal?; end

  # Flush the logging device. Messages are not guaranteed to be written until this method is called.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:266
  def flush; end

  # Get the Lumberjack::Formatter used to format objects for logging as messages.
  #
  # @return [Lumberjack::Formatter] The formatter.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:159
  def formatter; end

  # Set the Lumberjack::Formatter used to format objects for logging as messages.
  #
  # @param [Lumberjack::Formatter, Object] value The formatter to use.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:152
  def formatter=(value); end

  # Return true if the thread is currently in a Lumberjack::Context block.
  # When the logger is in a context block, tagging will only apply to that block.
  #
  # @return [Boolean]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:613
  def in_tag_context?; end

  # Log an +INFO+ message. The message can be passed in either the +message+ argument or in a block.
  #
  # @param [Object] message_or_progname_or_tags The message to log or progname
  #   if the message is passed in a block.
  # @param [String] progname_or_tags The name of the program that is logging the message or tags
  #   if the message is passed in a block.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:378
  def info(message_or_progname_or_tags = T.unsafe(nil), progname_or_tags = T.unsafe(nil), &block); end

  # Set the log level to info.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:392
  def info!; end

  # Return +true+ if +INFO+ messages are being logged.
  #
  # @return [Boolean]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:385
  def info?; end

  # The time that the device was last flushed.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:31
  def last_flushed_at; end

  # Get the level of severity of entries that are logged. Entries with a lower
  # severity level will be ignored.
  #
  # @return [Integer] The severity level.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:123
  def level; end

  # Set the log level using either an integer level like Logger::INFO or a label like
  # :info or "info"
  #
  # @param [Integer, Symbol, String] value The severity level.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:134
  def level=(value); end

  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:261
  def log(severity, message = T.unsafe(nil), progname = T.unsafe(nil), &block); end

  # Provided for compatibility with ActiveSupport::LoggerThreadSafeLevel to temporarily set the log level.
  #
  # @param [Integer, String, Symbol] level The log level to use inside the block.
  # @return [Object] The result of the block.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:468
  def log_at(level, &block); end

  # The Formatter used only for log entry messages.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:40
  def message_formatter; end

  # The Formatter used only for log entry messages.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:40
  def message_formatter=(_arg0); end

  # Set the name of the program to attach to log entries.
  # Get the program name associated with log messages.
  #
  # @return [String]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:497
  def progname; end

  # Set the name of the program to attach to log entries.
  # Get the program name associated with log messages.
  #
  # @return [String]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:37
  def progname=(_arg0); end

  # Remove a tag from the current tag context. If this is called inside a tag context,
  # the tags will only be removed for the duration of that block. Otherwise they will be removed
  # from the global tags.
  #
  # @param [Array<String, Symbol>] tag_names The tags to remove.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:561
  def remove_tag(*tag_names); end

  # Reopen the logging device.
  #
  # @param [Object] logdev passed through to the logging device.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:291
  def reopen(logdev = T.unsafe(nil)); end

  # Set the program name that is associated with log messages. If a block
  # is given, the program name will be valid only within the block.
  #
  # @param [String] value The program name to use.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:477
  def set_progname(value, &block); end

  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:127
  def sev_threshold; end

  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:138
  def sev_threshold=(value); end

  # Silence the logger by setting a new log level inside a block. By default, only +ERROR+ or +FATAL+
  # messages will be logged.
  #
  # @param [Integer, String, Symbol] temporary_level The log level to use inside the block.
  # @return [Object] The result of the block.
  #
  # @example
  #
  #   logger.level = Logger::INFO
  #   logger.silence do
  #     do_something   # Log level inside the block is +ERROR+
  #   end
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:453
  def silence(temporary_level = T.unsafe(nil), &block); end

  # Set +silencer+ to false to disable silencing the log.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:34
  def silencer; end

  # Set +silencer+ to false to disable silencing the log.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:34
  def silencer=(_arg0); end

  # Set a hash of tags on logger. If a block is given, the tags will only be set
  # for the duration of the block. Otherwise the tags will be applied on the current
  # logger context for the duration of that context.
  #
  # If there is no block or context, the tags will be applied to the global context.
  # This behavior is deprecated. Use the `tag_globally` method to set global tags instead.
  #
  # @param [Hash] tags The tags to set.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:510
  def tag(tags, &block); end

  # The TagFormatter used for formatting tags for output
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:43
  def tag_formatter; end

  # The TagFormatter used for formatting tags for output
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:43
  def tag_formatter=(_arg0); end

  # Add global tags to the logger that will appear on all log entries.
  #
  # @param [Hash] tags The tags to set.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:550
  def tag_globally(tags); end

  # Get the value of a tag by name from the current tag context.
  #
  # @param [String, Symbol] name The name of the tag to get.
  # @return [Object, nil] The value of the tag or nil if the tag does not exist.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:584
  def tag_value(name); end

  # Enable this logger to function like an ActiveSupport::TaggedLogger. This will make the logger
  # API compatible with ActiveSupport::TaggedLogger and is provided as a means of compatibility
  # with other libraries that assume they can call the `tagged` method on a logger to add tags.
  #
  # The tags added with this method are just strings so they are stored in the logger tags
  # in an array under the "tagged" tag. So calling `logger.tagged("foo", "bar")` will result
  # in tags `{"tagged" => ["foo", "bar"]}`.
  #
  # @return [Lumberjack::Logger] self.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:177
  def tagged_logger!; end

  # Return all tags in scope on the logger including global tags set on the Lumberjack
  # context, tags set on the logger, and tags set on the current block for the logger.
  #
  # @return [Hash]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:570
  def tags; end

  # Log a message when the severity is not known. Unknown messages will always appear in the log.
  # The message can be passed in either the +message+ argument or in a block.
  #
  # @param [Object] message_or_progname_or_tags The message to log or progname
  #   if the message is passed in a block.
  # @param [String, Hash] progname_or_tags The name of the program that is logging the message or tags
  #   if the message is passed in a block.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:429
  def unknown(message_or_progname_or_tags = T.unsafe(nil), progname_or_tags = T.unsafe(nil), &block); end

  # Remove all tags on the current logger and logging context within a block.
  # You can still set new block scoped tags within theuntagged block and provide
  # tags on individual log methods.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:594
  def untagged(&block); end

  # Log a +WARN+ message. The message can be passed in either the +message+ argument or in a block.
  #
  # @param [Object] message_or_progname_or_tags The message to log or progname
  #   if the message is passed in a block.
  # @param [String, Hash] progname_or_tags The name of the program that is logging the message or tags
  #   if the message is passed in a block.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:353
  def warn(message_or_progname_or_tags = T.unsafe(nil), progname_or_tags = T.unsafe(nil), &block); end

  # Set the log level to warn.
  #
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:367
  def warn!; end

  # Return +true+ if +WARN+ messages are being logged.
  #
  # @return [Boolean]
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:360
  def warn?; end

  # Adjust the log level during the block execution for the current Fiber only.
  #
  # @param [Integer, Symbol, String] severity The severity level.
  # @return [Object] The result of the block.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:144
  def with_level(severity, &block); end

  # Set the logger progname for the duration of the block.
  #
  # @yield [Object] The block to execute with the program name set.
  # @param [String] value The program name to use.
  # @return [Object] The result of the block.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:490
  def with_progname(value, &block); end

  private

  # Dereference arguments to log calls so we can have methods with compatibility with ::Logger
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:620
  def call_add_entry(severity, message_or_progname_or_tags, progname_or_tags, &block); end

  # Create a thread that will periodically call flush.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:719
  def create_flusher_thread(flush_seconds); end

  # Merge a tags hash into an existing tags hash.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:645
  def merge_tags(current_tags, tags); end

  # Open a logging device.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:688
  def open_device(device, options); end

  # Set a local value for a thread tied to this object within a block.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:677
  def push_thread_local_value(name, value); end

  # Set a local value for a thread tied to this object.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:656
  def set_thread_local_value(name, value); end

  # Get a local value for a thread tied to this object.
  #
  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:671
  def thread_local_value(name); end

  # pkg:gem/lumberjack#lib/lumberjack/logger.rb:709
  def write_to_device(entry); end
end

# pkg:gem/lumberjack#lib/lumberjack/rack.rb:4
module Lumberjack::Rack; end

# Middleware to create a global context for Lumberjack for the scope of a rack request.
#
# The optional `env_tags` parameter can be used to set up global tags from the request
# environment. This is useful for setting tags that are relevant to the entire request
# like the request id, host, etc.
#
# pkg:gem/lumberjack#lib/lumberjack/rack/context.rb:10
class Lumberjack::Rack::Context
  # @param [Object] app The rack application.
  # @param [Hash] env_tags A hash of tags to set from the request environment. If a tag value is
  #   a Proc, it will be called with the request `env` as an argument to allow dynamic tag values
  #   based on request data.
  #
  # pkg:gem/lumberjack#lib/lumberjack/rack/context.rb:15
  def initialize(app, env_tags = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/rack/context.rb:20
  def call(env); end

  private

  # pkg:gem/lumberjack#lib/lumberjack/rack/context.rb:29
  def apply_tags(env); end
end

# Support for using the Rails ActionDispatch request id in the log.
# The format is expected to be a random UUID and only the first chunk is used for terseness
# if the abbreviated argument is true.
#
# @deprecated Use tags instead of request id for unit of work. Will be removed in version 2.0.
#
# pkg:gem/lumberjack#lib/lumberjack/rack/request_id.rb:10
class Lumberjack::Rack::RequestId
  # pkg:gem/lumberjack#lib/lumberjack/rack/request_id.rb:13
  def initialize(app, abbreviated = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/rack/request_id.rb:20
  def call(env); end
end

# pkg:gem/lumberjack#lib/lumberjack/rack/request_id.rb:11
Lumberjack::Rack::RequestId::REQUEST_ID = T.let(T.unsafe(nil), String)

# @deprecated Use the Lumberjack::Rack::Context middleware instead to set a global tag
# with an identifier to tie log entries together in a unit of work. Will be removed in version 2.0.
#
# pkg:gem/lumberjack#lib/lumberjack/rack/unit_of_work.rb:7
class Lumberjack::Rack::UnitOfWork
  # pkg:gem/lumberjack#lib/lumberjack/rack/unit_of_work.rb:8
  def initialize(app); end

  # pkg:gem/lumberjack#lib/lumberjack/rack/unit_of_work.rb:14
  def call(env); end
end

# The standard severity levels for logging messages.
#
# pkg:gem/lumberjack#lib/lumberjack/severity.rb:5
module Lumberjack::Severity
  class << self
    # Coerce a value to a severity level.
    #
    # @param [Integer, String, Symbol] value The value to coerce.
    # @return [Integer] The severity level.
    #
    # pkg:gem/lumberjack#lib/lumberjack/severity.rb:37
    def coerce(value); end

    # Convert a severity label to a level.
    #
    # @param [String, Symbol] label The severity label to convert.
    # @return [Integer] The severity level.
    #
    # pkg:gem/lumberjack#lib/lumberjack/severity.rb:29
    def label_to_level(label); end

    # Convert a severity level to a label.
    #
    # @param [Integer] severity The severity level to convert.
    # @return [String] The severity label.
    #
    # pkg:gem/lumberjack#lib/lumberjack/severity.rb:21
    def level_to_label(severity); end
  end
end

# Backward compatibilty with 1.0 API
#
# pkg:gem/lumberjack#lib/lumberjack/severity.rb:7
Lumberjack::Severity::DEBUG = T.let(T.unsafe(nil), Integer)

# pkg:gem/lumberjack#lib/lumberjack/severity.rb:10
Lumberjack::Severity::ERROR = T.let(T.unsafe(nil), Integer)

# pkg:gem/lumberjack#lib/lumberjack/severity.rb:11
Lumberjack::Severity::FATAL = T.let(T.unsafe(nil), Integer)

# pkg:gem/lumberjack#lib/lumberjack/severity.rb:8
Lumberjack::Severity::INFO = T.let(T.unsafe(nil), Integer)

# pkg:gem/lumberjack#lib/lumberjack/severity.rb:14
Lumberjack::Severity::SEVERITY_LABELS = T.let(T.unsafe(nil), Array)

# pkg:gem/lumberjack#lib/lumberjack/severity.rb:12
Lumberjack::Severity::UNKNOWN = T.let(T.unsafe(nil), Integer)

# pkg:gem/lumberjack#lib/lumberjack/severity.rb:9
Lumberjack::Severity::WARN = T.let(T.unsafe(nil), Integer)

# A tag context provides an interface for manipulating a tag hash.
#
# pkg:gem/lumberjack#lib/lumberjack/tag_context.rb:5
class Lumberjack::TagContext
  # pkg:gem/lumberjack#lib/lumberjack/tag_context.rb:6
  def initialize(tags); end

  # Get a tag value.
  #
  # @param name [String, Symbol] The tag key.
  # @return [Object] The tag value.
  #
  # pkg:gem/lumberjack#lib/lumberjack/tag_context.rb:25
  def [](name); end

  # Set a tag value.
  #
  # @param name [String, Symbol] The tag name.
  # @param value [Object] The tag value.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/tag_context.rb:51
  def []=(name, value); end

  # Remove tags from the context.
  #
  # @param names [Array<String, Symbol>] The tag names to remove.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/tag_context.rb:63
  def delete(*names); end

  # Merge new tags into the context tags. Tag values will be flattened using dot notation
  # on the keys. So `{ a: { b: 'c' } }` will become `{ 'a.b' => 'c' }`.
  #
  # If a block is given, then the tags will only be added for the duration of the block.
  #
  # @param tags [Hash] The tags to set.
  # @return [void]
  #
  # pkg:gem/lumberjack#lib/lumberjack/tag_context.rb:17
  def tag(tags); end

  # Return a copy of the tags as a hash.
  #
  # @return [Hash]
  #
  # pkg:gem/lumberjack#lib/lumberjack/tag_context.rb:74
  def to_h; end
end

# Class for formatting tags. You can register a default formatter and tag
# name specific formatters. Formatters can be either `Lumberjack::Formatter`
# objects or any object that responds to `call`.
#
# tag_formatter = Lumberjack::TagFormatter.new.default(Lumberjack::Formatter.new)
# tag_formatter.add(["password", "email"]) { |value| "***" }
# tag_formatter.add("finished_at", Lumberjack::Formatter::DateTimeFormatter.new("%Y-%m-%dT%H:%m:%S%z"))
#
# pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:11
class Lumberjack::TagFormatter
  # pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:12
  def initialize; end

  # Add a formatter for specific tag names or object classes. This can either be a Lumberjack::Formatter
  # or an object that responds to `call` or a block. The formatter will be applied if it matches either a tag name
  # or if the tag value is an instance of a registered class. Tag name formatters will take precedence
  # over class formatters. The default formatter will not be applied to a value if a tag formatter
  # is applied to it.
  #
  # Name formatters can be applied to nested hashes using dot syntax. For example, if you add a formatter
  # for "foo.bar", it will be applied to the value of the "bar" key in the "foo" tag if that value is a hash.
  #
  # Class formatters will be applied recursively to nested hashes and arrays.
  #
  # @param names_or_classes [String, Module, Array<String, Module>] The tag names or object classes
  #   to apply the formatter to.
  # @param formatter [Lumberjack::Formatter, #call, nil] The formatter to use.
  #    If this is nil, then the block will be used as the formatter.
  # @return [Lumberjack::TagFormatter] self
  #
  # @example
  #  tag_formatter.add("password", &:redact)
  #
  # pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:58
  def add(names_or_classes, formatter = T.unsafe(nil), &block); end

  # Remove all formatters.
  #
  # @return [Lumberjack::TagFormatter] self
  #
  # pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:93
  def clear; end

  # Add a default formatter applied to all tag values. This can either be a Lumberjack::Formatter
  # or an object that responds to `call` or a block.
  #
  # @param formatter [Lumberjack::Formatter, #call, nil] The formatter to use.
  #    If this is nil, then the block will be used as the formatter.
  # @return [Lumberjack::TagFormatter] self
  #
  # pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:24
  def default(formatter = T.unsafe(nil), &block); end

  # Format a hash of tags using the formatters
  #
  # @param tags [Hash] The tags to format.
  # @return [Hash] The formatted tags.
  #
  # pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:103
  def format(tags); end

  # Remove formatters for specific tag names. The default formatter will still be applied.
  #
  # @param names_or_classes [String, Module, Array<String, Module>] The tag names or classes to remove the formatter from.
  # @return [Lumberjack::TagFormatter] self
  #
  # pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:79
  def remove(names_or_classes); end

  # Remove the default formatter.
  #
  # @return [Lumberjack::TagFormatter] self
  #
  # pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:34
  def remove_default; end

  private

  # pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:180
  def class_formatter(klass); end

  # pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:169
  def dereference_formatter(formatter); end

  # pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:125
  def formatted_tag_value(name, value, skip_classes: T.unsafe(nil), prefix: T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/tag_formatter.rb:114
  def formatted_tags(tags, skip_classes: T.unsafe(nil), prefix: T.unsafe(nil)); end
end

# Methods to make Lumberjack::Logger API compatible with ActiveSupport::TaggedLogger.
#
# pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:8
module Lumberjack::TaggedLoggerSupport
  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:73
  def clear_tags!; end

  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:60
  def pop_tags(size = T.unsafe(nil)); end

  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:56
  def push_tags(*tags); end

  # Compatibility with ActiveSupport::TaggedLogging which only supports adding tags as strings.
  # Tags will be added to the "tagged" key in the logger's tags hash as an array.
  #
  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:42
  def tagged(*tags, &block); end
end

# pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:9
class Lumberjack::TaggedLoggerSupport::Formatter
  extend ::Forwardable

  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:13
  def initialize(formatter:, logger:); end

  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:35
  def __formatter; end

  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:11
  def clear_tags!(*args, **_arg1, &block); end

  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:19
  def current_tags; end

  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:11
  def pop_tags(*args, **_arg1, &block); end

  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:11
  def push_tags(*args, **_arg1, &block); end

  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:11
  def tagged(*args, **_arg1, &block); end

  # pkg:gem/lumberjack#lib/lumberjack/tagged_logger_support.rb:28
  def tags_text; end
end

# Monkey patch for ActiveSupport::TaggedLogger so it doesn't blow up when
# a Lumberjack logger is trying to be wrapped. This module will be automatically
# included in ActiveSupport::TaggedLogger if activesupport is already loaded.
#
# pkg:gem/lumberjack#lib/lumberjack/tagged_logging.rb:7
module Lumberjack::TaggedLogging
  mixes_in_class_methods ::Lumberjack::TaggedLogging::ClassMethods

  class << self
    # pkg:gem/lumberjack#lib/lumberjack/tagged_logging.rb:9
    def included(base); end
  end
end

# pkg:gem/lumberjack#lib/lumberjack/tagged_logging.rb:14
module Lumberjack::TaggedLogging::ClassMethods
  # pkg:gem/lumberjack#lib/lumberjack/tagged_logging.rb:15
  def new(logger); end
end

# pkg:gem/lumberjack#lib/lumberjack/tags.rb:4
class Lumberjack::Tags
  class << self
    # Ensure keys are strings and expand any values in a hash that are Proc's by calling them and replacing
    # the value with the result. This allows setting global tags with runtime values.
    #
    # @param [Hash] hash The hash to transform.
    # @return [Hash] The hash with string keys and expanded values.
    #
    # pkg:gem/lumberjack#lib/lumberjack/tags.rb:25
    def expand_runtime_values(hash); end

    # Transform hash keys to strings. This method exists for optimization and backward compatibility.
    # If a hash already has string keys, it will be returned as is.
    #
    # @param [Hash] hash The hash to transform.
    # @return [Hash] The hash with string keys.
    #
    # pkg:gem/lumberjack#lib/lumberjack/tags.rb:11
    def stringify_keys(hash); end
  end
end

# A template converts entries to strings. Templates can contain the following place holders to
# reference log entry values:
#
# * :time
# * :severity
# * :progname
# * :tags
# * :message
#
# Any other words prefixed with a colon will be substituted with the value of the tag with that name.
# If your tag name contains characters other than alpha numerics and the underscore, you must surround it
# with curly brackets: `:{http.request-id}`.
#
# pkg:gem/lumberjack#lib/lumberjack/template.rb:16
class Lumberjack::Template
  # Create a new template from the markup. The +first_line+ argument is used to format only the first
  # line of a message. Additional lines will be added to the message unformatted. If you wish to format
  # the additional lines, use the :additional_lines options to specify a template. Note that you'll need
  # to provide the line separator character in this template if you want to keep the message on multiple lines.
  #
  # The time will be formatted as YYYY-MM-DDTHH:MM:SSS.SSS by default. If you wish to change the format, you
  # can specify the :time_format option which can be either a time format template as documented in
  # +Time#strftime+ or the values +:milliseconds+ or +:microseconds+ to use the standard format with the
  # specified precision.
  #
  # Messages will have white space stripped from both ends.
  #
  # @param [String] first_line The template to use to format the first line of a message.
  # @param [Hash] options The options for the template.
  #
  # pkg:gem/lumberjack#lib/lumberjack/template.rb:36
  def initialize(first_line, options = T.unsafe(nil)); end

  # Convert an entry into a string using the template.
  #
  # @param [Lumberjack::LogEntry] entry The entry to convert to a string.
  # @return [String] The entry converted to a string.
  #
  # pkg:gem/lumberjack#lib/lumberjack/template.rb:68
  def call(entry); end

  # Get the format used to format the time.
  #
  # @return [String]
  #
  # pkg:gem/lumberjack#lib/lumberjack/template.rb:60
  def datetime_format; end

  # Set the format used to format the time.
  #
  # @param [String] format The format to use to format the time.
  #
  # pkg:gem/lumberjack#lib/lumberjack/template.rb:48
  def datetime_format=(format); end

  private

  # Compile the template string into a value that can be used with sprintf.
  #
  # pkg:gem/lumberjack#lib/lumberjack/template.rb:118
  def compile(template); end

  # pkg:gem/lumberjack#lib/lumberjack/template.rb:98
  def tag_args(tags, tag_vars); end
end

# pkg:gem/lumberjack#lib/lumberjack/template.rb:19
Lumberjack::Template::MICROSECOND_TIME_FORMAT = T.let(T.unsafe(nil), String)

# pkg:gem/lumberjack#lib/lumberjack/template.rb:18
Lumberjack::Template::MILLISECOND_TIME_FORMAT = T.let(T.unsafe(nil), String)

# pkg:gem/lumberjack#lib/lumberjack/template.rb:20
Lumberjack::Template::PLACEHOLDER_PATTERN = T.let(T.unsafe(nil), Regexp)

# pkg:gem/lumberjack#lib/lumberjack/template.rb:17
Lumberjack::Template::TEMPLATE_ARGUMENT_ORDER = T.let(T.unsafe(nil), Array)

# pkg:gem/lumberjack#lib/lumberjack/utils.rb:6
module Lumberjack::Utils
  class << self
    # Print warning when deprecated methods are called the first time. This can be disabled
    # by setting the environment variable `LUMBERJACK_NO_DEPRECATION_WARNINGS` to "true".
    # You can see every usage of a deprecated method along with a full stack trace by setting
    # the environment variable `VERBOSE_LUMBERJACK_DEPRECATION_WARNING` to "true".
    #
    # @param method [String] The name of the deprecated method.
    # @param message [String] Optional message to include in the warning.
    # @yield The block to execute after the warning.
    #
    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:26
    def deprecated(method, message); end

    # Expand a hash of tags that may contain nested hashes or dot notation keys. Dot notation tags
    # will be expanded into nested hashes.
    #
    # @param tags [Hash] The hash of tags to expand.
    # @return [Hash] The expanded hash with dot notation keys.
    #
    # @example
    #   expand_tags({"user.id" => 123, "user.name" => "Alice", "action" => "login"})
    #   # => {"user" => {"id" => 123, "name" => "Alice"}, "action" => "login"}
    #
    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:125
    def expand_tags(tags); end

    # Flatten a tag hash to a single level hash with dot notation for nested keys.
    #
    # @param tag_hash [Hash] The hash to flatten.
    # @return [Hash<String, Object>] The flattened hash.
    # @example
    #   expand_tags(user: {id: 123, name: "Alice"}, action: "login")})
    #   # => {"user.id" => 123, "user.name" => "Alice", "action" => "login"}
    #
    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:110
    def flatten_tags(tag_hash); end

    # Force encode a string to UTF-8. Any invalid byte sequences will be
    # ignored and replaced with an empty string.
    #
    # @param str [String] The string to encode.
    # @return [String] The UTF-8 encoded string.
    #
    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:97
    def force_utf8(str); end

    # Generate a global process ID that includes the hostname and process ID.
    #
    # @return [String] The global process ID.
    #
    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:67
    def global_pid; end

    # Generate a global thread ID that includes the global process ID and the thread name.
    #
    # @return [String] The global thread ID.
    #
    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:78
    def global_thread_id; end

    # Get the hostname of the machine. The returned value will be in UTF-8 encoding.
    #
    # @return [String] The hostname of the machine.
    #
    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:49
    def hostname; end

    # Set the hostname to a specific value. If this is not specified, it will use the system hostname.
    #
    # @param hostname [String]
    # @return [void]
    #
    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:60
    def hostname=(hostname); end

    # Get the name of a thread. The value will be based on the thread's name if it exists.
    # Otherwise a unique id is generated based on the thread's object id. Only alphanumeric
    # characters, underscores, dashes, and periods are kept in thread name.
    #
    # @param thread [Thread] The thread to get the name for. Defaults to the current thread.
    # @return [String] The name of the thread.
    #
    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:88
    def thread_name(thread = T.unsafe(nil)); end

    private

    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:153
    def expand_dot_notation_hash(hash, expanded = T.unsafe(nil)); end

    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:133
    def flatten_hash_recursive(hash, prefix = T.unsafe(nil)); end

    # pkg:gem/lumberjack#lib/lumberjack/utils.rb:144
    def slugify(str); end
  end
end

# pkg:gem/lumberjack#lib/lumberjack/utils.rb:10
Lumberjack::Utils::NON_SLUGGABLE_PATTERN = T.let(T.unsafe(nil), Regexp)

# pkg:gem/lumberjack#lib/lumberjack/utils.rb:7
Lumberjack::Utils::UNDEFINED = T.let(T.unsafe(nil), Object)
