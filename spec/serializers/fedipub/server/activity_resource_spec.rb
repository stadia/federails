...
RSpec.describe Fedipub::Server::ActivityResource do
  ...
  context 'when rendering a Like activity' do
    ...
    let(:activity) do
      FactoryBot.create(
        ...
        entity: post,
        action: 'Like',
        to:     [post.fedipub_actor.federated_url],
        cc:     []
      )
    end
  ...
  context 'when rendering an Update activity for an actor' do
    ...
    let(:activity) do
      ...
      Fedipub::Activity.where(action: 'Update').order(:id).last
    end
    let(:json_result) { render_activity(activity) }
    ...
    it 'embeds the updated actor object' do
      aggregate_failures do
        expect(json_result['object']).to be_a(Hash)
        expect(json_result.dig('object', 'id')).to eq(entity.fedipub_actor.federated_url)
        expect(json_result.dig('object', 'type')).to eq(entity.fedipub_actor.actor_type)
        expect(json_result.dig('object', 'preferredUsername')).to eq(entity.fedipub_actor.username)
