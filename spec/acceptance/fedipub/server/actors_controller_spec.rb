...
RSpec.describe Fedipub::Server::ActorsController, type: :acceptance do
  ...
  let(:following) do
    FactoryBot.create_list(:user, 2).each do |user|
      relationship = Fedipub::Following.create actor: actor, target_actor: user.fedipub_actor
      relationship.accept!(follow_activity: relationship.follow_activity)
    end
  end
  let(:followers) do
    FactoryBot.create_list(:user, 2).each do |user|
      relationship = Fedipub::Following.create actor: user.fedipub_actor, target_actor: actor
      relationship.accept!(follow_activity: relationship.follow_activity)
    end
  end
