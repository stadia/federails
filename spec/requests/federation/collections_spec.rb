...
RSpec.describe 'Federation collections (liked, featured, featured_tags)', type: :request do
  ...
  describe 'GET /liked' do
    it 'renders a successful response' do
      get fedipub.liked_server_actor_url(actor), headers: { accept: Mime[:activitypub] }
    ...
    context 'with liked activities' do
      ...
      before do
        Fedipub::Activity.create!(actor: actor, entity: post, action: 'Like')
      end

      it 'includes liked object URLs on page' do
        get fedipub.liked_server_actor_url(actor, page: 1), headers: { accept: Mime[:activitypub] }
        json = JSON.parse(response.body) # rubocop:disable Rails/ResponseParsedBody
  ...
  describe 'GET /featured' do
    it 'renders a successful response' do
      get fedipub.featured_server_actor_url(actor), headers: { accept: Mime[:activitypub] }
      expect(response).to be_successful
    end
    ...
    context 'with featured items' do
      ...
      it 'includes featured items on page' do
        get fedipub.featured_server_actor_url(actor, page: 1), headers: { accept: Mime[:activitypub] }
        json = JSON.parse(response.body) # rubocop:disable Rails/ResponseParsedBody
  ...
  describe 'GET /featured_tags' do
    it 'renders a successful response' do
      get fedipub.featured_tags_server_actor_url(actor), headers: { accept: Mime[:activitypub] }
      expect(response).to be_successful
    end
    ...
    context 'with featured tags' do
      ...
      it 'includes featured tags on page' do
        get fedipub.featured_tags_server_actor_url(actor, page: 1), headers: { accept: Mime[:activitypub] }
        json = JSON.parse(response.body) # rubocop:disable Rails/ResponseParsedBody
  ...
  describe 'Actor JSON' do
    it 'includes liked, featured, and featuredTags URLs' do
      get fedipub.server_actor_url(actor), headers: { accept: Mime[:activitypub] }
      json = JSON.parse(response.body) # rubocop:disable Rails/ResponseParsedBody
