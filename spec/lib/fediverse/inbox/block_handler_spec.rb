...
RSpec.describe Fediverse::Inbox::BlockHandler do
  ...
  describe '.handle_block' do
    ...
    it 'is idempotent' do
      ...
      expect { described_class.handle_block(activity) }
        .not_to change(Fedipub::Block, :count)
    end

    context 'when followings exist' do
      before do
        Fedipub::Following.create!(actor: distant_actor, target_actor: local_actor)
        Fedipub::Following.create!(actor: local_actor, target_actor: distant_actor)
      end

      it 'destroys followings in both directions' do
        expect { described_class.handle_block(activity) }
          .to change(Fedipub::Following, :count).by(-2)
      end
    end
  end
  ...
  describe '.handle_undo_block' do
    let!(:block) do
      Fedipub::Block.create!(actor: distant_actor, target_actor: local_actor)
    end
