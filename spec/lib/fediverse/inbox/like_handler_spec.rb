...
RSpec.describe Fediverse::Inbox::LikeHandler do
  describe '.handle_like' do
    ...
    it 'dispatches to the data entity hook' do
      ...
      allow(entity).to receive(:run_callbacks).with(:on_fedipub_like_received).and_yield

      expect(described_class.handle_like(activity)).to be true
      expect(entity).to have_received(:run_callbacks).with(:on_fedipub_like_received)
    end
  ...
  describe '.handle_undo_like' do
    ...
    it 'dispatches to the data entity undo hook' do
      ...
      allow(Fedipub::Utils::Object).to receive(:find_or_initialize).with(entity.federated_url).and_return(entity)
      allow(entity).to receive(:run_callbacks).with(:on_fedipub_undo_like_received).and_yield

      expect(described_class.handle_undo_like(activity)).to be true
      expect(entity).to have_received(:run_callbacks).with(:on_fedipub_undo_like_received)
    end
