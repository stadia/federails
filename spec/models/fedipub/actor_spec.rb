  ...
  RSpec.describe Actor, type: :model do
    ...
    describe '.shared_inbox_url' do
      context 'with a local actor' do
        it 'returns the server shared inbox route' do
          expected_url = Fedipub::Engine.routes.url_helpers.server_shared_inbox_url
    ...
    describe 'accepted relationships' do
      ...
      before do
        ...
        accepted_inbound_alice = Fedipub::Following.create! actor: alice_follower, target_actor: actor
        accepted_inbound_alice.accept!(follow_activity: accepted_inbound_alice.follow_activity)
        accepted_inbound_bob = Fedipub::Following.create! actor: bob_follower, target_actor: actor
        accepted_inbound_bob.accept!(follow_activity: accepted_inbound_bob.follow_activity)

        # Create pending followers
        Fedipub::Following.create! actor: pending_follower, target_actor: actor

        # Create accepted followings
        accepted_outbound_carol = Fedipub::Following.create! actor: actor, target_actor: carol_following
        accepted_outbound_carol.accept!(follow_activity: accepted_outbound_carol.follow_activity)
        accepted_outbound_dave = Fedipub::Following.create! actor: actor, target_actor: dave_following
        accepted_outbound_dave.accept!(follow_activity: accepted_outbound_dave.follow_activity)

        # Create pending followings
        Fedipub::Following.create! actor: actor, target_actor: pending_following
      end
      ...
      describe '#accepted_followers' do
        ...
        it 'excludes pending followers' do
          expect(actor.accepted_followers.pluck(:id)).not_to include(
            Fedipub::Following.where(target_actor: actor, status: :pending).first.actor_id
          )
        end
      end
      ...
      describe '#accepted_follows' do
        ...
        it 'excludes pending followings' do
          expect(actor.accepted_follows.pluck(:id)).not_to include(
            Fedipub::Following.where(actor: actor, status: :pending).first.target_actor_id
          )
        end
      end
